...
//     try {
//       const res = await axios.get(`https://smithtask-server.vercel.app/api/cart/${user.id}`);
...
//     try {
//       await axios.delete(`https://smithtask-server.vercel.app/api/cart/${user.id}/${productId}`);
...
//     if (qty < 1) return;
//     try {
//       await axios.put(`https://smithtask-server.vercel.app/api/cart/${user.id}`, {
//         productId,
//         qty,
//       });
//       setCartItems(
...
const CartList = () => {
  ...
  const fetchCartItems = useCallback(async () => {
    ...
    try {
      const res = await axios.get(
        `https://smithtask-server.vercel.app/api/cart/${user.id}`
      );
      setCartItems(res.data);
    } catch (err) {
      console.error("❌ Error fetching cart:", err);
  ...
  const removeItem = async (productId) => {
    try {
      await axios.delete(
        `https://smithtask-server.vercel.app/api/cart/${user.id}/${productId}`
      );
      setCartItems((prev) =>
  ...
  const totalDays = cartItems.reduce((sum, item) => {
    const totalDuration = item.makingDuration * item.quantity;
    return sum + Math.ceil(totalDuration / 8);
  }, 0);

  if (!user) {
    return <p className="text-center mt-5">⚠️ Please login to view cart.</p>;
  }
      ...
      {cartItems.length === 0 ? (
        ...
        <>
            ...
            <tbody>
              {cartItems.map((item) => {
                const days = Math.ceil(
                  (item.makingDuration * item.quantity) / 8
                );
          ...
          <div className="d-flex justify-content-between align-items-center mt-3">
            ...
            <h5>
              Total Making Days: <strong>{totalDays}</strong>
            </h5>
